var TransactionFeed = function (options) {
  ...
  this.handleTransaction = function (data) {
    ...
    rows.select('.time').html(function (d) {
      return self.absoluteTime(d.time);
    });
    rows.select('.icon').attr('class', function (d) {
      return d.type + ' icon';
    }).attr('title', function (d) {
      return txAltText[d.type];
    });
    rows.select('.details').html(function (d) {
      var html = self.prepareAmount(d.amount1, d.currency1);
      if (d.amount2)
        html += '<i>for</i>' + self.prepareAmount(d.amount2, d.currency2);
      return html;
    });
  ...
  this.prepareAmount = function (amount, currency) {
    var html = '';
    if (amount && currency)
      html = '<b>' + self.commas(amount) + '</b><small>' + currency + '</small>';
    return html;
  };
  ...
  this.absoluteDate = function (secondsSince2000) {
    var d = new Date(0);
    d.setUTCSeconds(secondsSince2000 + 946684800);
    return '<span class="date" title="' + self.absoluteTimeOnly(secondsSince2000) + '">' + self.absoluteDateOnly(secondsSince2000) + '</span>';
  };
  this.absoluteTime = function (secondsSince2000) {
    var d = new Date(0);
    d.setUTCSeconds(secondsSince2000 + 946684800);
    return '<span class="date" title="' + self.absoluteDateOnly(secondsSince2000) + '">' + self.absoluteTimeOnly(secondsSince2000) + '</span>';
  };
