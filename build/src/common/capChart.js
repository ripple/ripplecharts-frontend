function CapChart(options) {
  var self = this, apiHandler = new ApiHandler(options.url);
  ...
  if (!options.width)
    options.width = parseInt(chart.style('width'), 10) - options.margin.left - options.margin.right;
  if (!options.height)
  ...
  dropdowns.append('div').attr('class', 'dropdowns dataType').append('select').selectAll('option').data([
    'Capitalization',
    'Transaction Volume',
    'Trade Volume'
  ...
  dropdowns.select('.dataType select').on('change', function () {
    ...
    if (self.currency == 'XRP')
      self.currency = dropdowns.select('.currency').node().value;
  ...
  if (self.dataType == 'Transaction Volume')
    currencyList.unshift('XRP');
  var currencyDropdown = ripple.currencyDropdown(currencyList).selected({ currency: self.currency }).on('change', function (currency) {
      self.currency = currency;
      var range = controls.select('.interval .selected').datum();
      loadData(range);
    });
  dropdowns.append('div').attr('class', 'dropdowns').call(currencyDropdown);
  var type = controls.append('div').attr('class', 'chartType selectList').selectAll('a').data([
      'line',
      'stacked'
      ...
      var that = this;
      type.classed('selected', function () {
        return this === that;
      });
      self.format = d;
      drawData();
  ...
  var xScale = d3.time.scale(), yScale = d3.scale.linear(), color = d3.scale.category20(), xAxis = d3.svg.axis().scale(xScale).orient('bottom'), yAxis = d3.svg.axis().scale(yScale).orient('right').tickFormat(d3.format('s'));
  ...
  var svg, g, timeAxis, amountAxis, amountLabel, borders, sections, lines, tracer, tooltip, status, loader, isLoading;
  var capDataCache = {};
  var sendDataCache = {};
  var tradeDataCache = {};
  ...
  function loadSendData(range) {
    if (sendDataCache[self.currency] && sendDataCache[self.currency][self.range]) {
      isLoading = false;
      drawData();
      drawLegend();
      return;
  ...
  function loadSendDataHelper(range, c, count) {
    ...
    apiHandler.valueSent({
      ...
      if (self.dataType == 'Transaction Volume' && self.currency == currency && self.range == range.name) {
  ...
  function loadTradeData(range) {
    if (tradeDataCache[self.currency] && tradeDataCache[self.currency][self.range]) {
      isLoading = false;
      drawData();
      drawLegend();
      return;
  ...
  function loadTradeHelper(range, base, count) {
    ...
    apiHandler.offersExercised({
      ...
      prepareStackedData(currency, range);
      prepareLegend(currency, range);
      if ((self.dataType == 'Trade Volume' || self.dataType == '# of Trades') && self.currency == currency && self.range == range.name) {
  ...
  function loadCapitalizationData(range) {
    var currency = self.currency;
    if (capDataCache[currency] && capDataCache[currency][range.name]) {
      isLoading = false;
      drawData();
      drawLegend();
      return;
    ...
    apiHandler.issuerCapitalization({
      ...
      if (!capDataCache[self.currency])
        capDataCache[self.currency] = {};
      capDataCache[self.currency][self.range] = { raw: data };
      data.forEach(function (d, index) {
        d.results = d.results.map(function (d) {
          return [
            moment.utc(d[0]).unix() * 1000,
            d[1]
          ];
      ...
      prepareStackedData(currency, range);
      prepareLegend(currency, range);
      if (self.dataType == 'Capitalization' && self.currency == currency && self.range == range.name) {
        isLoading = false;
        drawData();
        drawLegend();
      }
  ...
  function prepareLegend(currency, range) {
    ...
    legend = raw.map(function (d) {
      ...
      if (d.values)
        amount = d.values.length ? commas(d.values[d.values.length - 1].y, 2) : 0;
      else
        amount = d.results.length ? commas(d.results[d.results.length - 1][1], 2) : 0;
      return {
  ...
  function drawChart() {
    ...
    g.append('rect').attr('class', 'background').attr('width', options.width).attr('height', options.height).on('mousemove', movingInSky);
    timeAxis = svg.append('g').attr('class', 'x axis').attr('transform', 'translate(' + options.margin.left + ',' + (options.height + options.margin.top) + ')');
    amountAxis = svg.append('g').attr('class', 'y axis').attr('transform', 'translate(' + (options.width + options.margin.left) + ',' + options.margin.top + ')');
    amountLabel = amountAxis.append('text').attr('class', 'title').attr('transform', 'rotate(-90)').attr('y', -5).attr('x', -options.height * 0.85);
    xScale.range([
      0,
      options.width
    ]);
    ...
    loader = chart.append('img').attr('class', 'loader').attr('src', 'assets/images/rippleThrobber.png');
    if (isLoading)
  ...
  function drawData() {
    ...
    if (self.format == 'stacked') {
      ...
      if (data.length) {
        ...
        section.enter().append('g').attr('class', 'section').style('display', function (d) {
          return d.hide ? 'none' : 'inherit';
        });
        section.exit().remove();
        var path = section.selectAll('path').data(function (d) {
            return [d];
          });
      ...
      p.enter().append('path').attr('class', 'l').on('mouseover', movingOnLine);
      ...
      p.exit().remove();
      var a = line.selectAll('.a').data(function (d) {
          return [d];
        });
  ...
  function drawLegend() {
    ...
    if (self.dataType == 'Capitalization')
      items.select('.amount').html(function (d) {
  ...
  function movingInSky() {
    ...
    if (self.format == 'stacked') {
      ...
      rows = lines.map(function (d, i) {
        j = d3.bisectLeft(d.results.map(function (d) {
          return d[0];
        }), date);
        return [
          i,
          j,
          d.results[j] ? d.results[j][1] : 0
        ];
      });
  ...
  function movingOnLine(line) {
    ...
    var zoom = chart.style('zoom') || 1;
    var date = xScale.invert(d3.mouse(this)[0] / zoom);
    var i = d3.bisect(line.results.map(function (d) {
        return d[0];
      }), date);
    if (i && date < (line.results[i][0] + line.results[i - 1][0]) / 2)
      i--;
  ...
  function handleTracer(cx, cy, c2y) {
    ...
    if (c2y) {
      tracer.select('.vertical').transition().duration(dur).attr({
        x1: cx,
        x2: cx,
        y1: cy,
        y2: c2y
      ...
      tracer.select('.bottom').transition().duration(dur).attr({
        cx: cx,
        cy: c2y
      }).style('opacity', 1);
    } else {
      ...
      tracer.select('.horizontal').transition().duration(dur).attr({
        x1: cx,
  ...
  function handleTooltip(title, address, date, amount, position) {
    ...
    if (address)
      tooltip.append('div').html(address).style('color', color(address)).attr('class', 'address');
    tooltip.append('div').html(date).attr('class', 'date');
    tooltip.append('div').html(amount + ' ' + self.currency).attr('class', 'amount');
    tooltip.transition().duration(100).style('left', position[0] + 'px').style('top', position[1] + 'px').style('opacity', 1);
  }
  ...
  function getExtents(axis, data) {
    var max, min, rows = data.map(function (d) {
        return d3.extent(d.results, function (d) {
          return axis == 'x' ? d[0] : d[1];
        });
      });
