var TotalHistory = function (options) {
  var request, basisRequest, ts, cp, filter, last, interval, c = ripple.currencyDropdown(), to_export = {}, issuers = {}, ctx = $('#canvas').get(0).getContext('2d'), apiHandler = new ApiHandler(options.url);
  ...
  var svgContainer = d3.select('.chart_wrapper').append('svg').attr('id', 'canvas2'), line = svgContainer.append('line').attr('stroke-width', 0).attr('class', 'line'), line2 = svgContainer.append('line').attr('stroke-width', 0).attr('class', 'line'), circle = svgContainer.append('circle').attr('r', 0).attr('class', 'circle'), xborder = svgContainer.append('line').attr('x1', 0).attr('y1', 9).attr('x2', '100%').attr('y2', 9).attr('stroke-width', 0).attr('class', 'border'), yborder = svgContainer.append('line').attr('x1', '100%').attr('y1', 9).attr('x2', '100%').attr('y2', '100%').attr('stroke-width', 0).attr('class', 'border');
  var chart_options = {
      responsive: true,
  ...
  var inc = 'day', end = moment().subtract(1, 'month').subtract(1, 'day').format('MM/DD/YYYY'), min = moment().subtract(1, 'month').format('MM/DD/YYYY'), start = moment().format('MM/DD/YYYY'), curr = 'USD';
  ...
  $('#canvas').mousemove(function (evt) {
    if ($('#loading').css('display') === 'none') {
      ...
      var activeBars = myLine.getPointsAtEvent(evt), text = '', c_point = {
