var TotalAccounts = function (options) {
  ...
  var interval = list.selectAll('a').data([
      ...
      },
      {
        ...
        interval: 'month',
        offset: function (d) {
          return new Date('Dec 31 2012 0:00:00');
        }
      }
  ...
  var chart = new LineChart({
      ...
      tooltip: function (d, increment) {
        return '<div>' + parseDate(d.x.local(), increment) + '</div><span>Accounts Created:</span>' + d.y2 + '<br/><span>Total Accounts:</span>' + d.y;
      }
    });
  ...
  function updateHelper(params) {
    ...
    request = apiHandler.accountsCreated(params, function (err, data) {
      ...
      var total = chart.basis, lineData = [];
      data.splice(0, 1);
      data.reverse();
  ...
  function parseDate(date, increment) {
    ...
    if (increment == 'month')
      ...
      return monthNames[date.month()] + ' ' + date.date();
    else if (increment == 'hour')
      return monthNames[date.month()] + ' ' + date.date() + ' &middot ' + date.format('hh:mm A');
    else
      return monthNames[date.month()] + ' ' + date.date() + ' &middot ' + date.format('hh:mm:ss A');
  }
