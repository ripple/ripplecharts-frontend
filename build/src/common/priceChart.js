if (typeof LOADER_PNG == 'undefined')
  ...
  LOADER_PNG = 'data:image/png;base64,' + LOADER_PNG;
PriceChart = function (options) {
  var self = this, xScale = d3.time.scale(), priceScale = d3.scale.linear(), volumeScale = d3.scale.linear(), xAxis = d3.svg.axis().scale(xScale), volumeAxis = d3.svg.axis().scale(volumeScale).orient('left').tickFormat(d3.format('s')), priceAxis = d3.svg.axis().scale(priceScale).orient('right'), apiHandler = new ApiHandler(options.url), liveFeed, isLoading;
  self.onStateChange = null;
  var wrap, div, h, w, staticHeight, svg, svgEnter, gEnter, gradient, hover, horizontal, focus, status, details, loader, base, counter, lineData = [], startTime, endTime, type, chartInterval, intervalSeconds, multiple, lastCandle;
  ...
  if (options.height)
    staticHeight = options.height - options.margin.top - options.margin.bottom;
  else if (h)
    staticHeight = options.height = h - options.margin.top - options.margin.bottom;
  else
  ...
  function drawChart() {
    ...
    svgEnter = svg.enter().append('svg').attr('width', options.width + options.margin.left + options.margin.right).attr('height', options.height + options.margin.top + options.margin.bottom);
    ...
    gEnter = svg.append('g').attr('transform', 'translate(' + options.margin.left + ',' + options.margin.top + ')');
    ...
    gEnter.append('g').attr('class', 'volume axis').append('text').text('Volume').attr('class', 'title').attr('transform', 'rotate(-90)').attr('y', 15).attr('x', -110);
    gEnter.append('g').attr('class', 'price axis').attr('transform', 'translate(' + options.width + ', 0)').append('text').text('Price').attr('class', 'title').attr('transform', 'rotate(-90)').attr('y', -10).attr('x', -100);
    gradient = svg.append('svg:defs').append('svg:linearGradient').attr('id', 'gradient').attr('x1', '0%').attr('y1', '0%').attr('x2', '0%').attr('y2', '100%').attr('spreadMethod', 'pad');
    gradient.append('svg:stop').attr('offset', '0%').attr('stop-color', '#ccc').attr('stop-opacity', 0.5);
    gradient.append('svg:stop').attr('offset', '100%').attr('stop-color', '#ddd').attr('stop-opacity', 0.5);
    ...
    details = div.append('div').attr('class', 'chartDetails').style('left', options.margin.left + 'px').style('right', options.margin.right + 'px').style('top', options.margin.top - 1 + 'px').style('opacity', 0);
    loader = div.append('img').attr('class', 'loader').attr('src', LOADER_PNG).style('opacity', 0);
    if (isLoading) {
  ...
  this.load = function (b, c, d) {
    if (!b)
      return setStatus('Base currency is required.');
    if (!c)
      return setStatus('Counter currency is required.');
    if (!d || !d.interval)
      return setStatus('Interval is required.');
    ...
    if (chartInterval == 'se')
      ...
      intervalSeconds = 60 * 60 * 24 * 30.5;
    else
      return setStatus('Invalid Interval');
  ...
  function drawData() {
    ...
    svg.datum(lineData, function (d) {
      return d.startTime;
    }).on('mousemove', showDetails).on('touchmove', showDetails).on('touchstart', showDetails).on('touchend', showDetails);
    xScale.domain([
      startTime,
      endTime
    ]).range([
    ...
    gEnter.select('.line').datum(lineData, function (d) {
      return d.startTime;
    }).transition().attr('d', line);
    var candle = gEnter.select('.candlesticks').selectAll('g').data(lineData, function (d) {
        return d.startTime;
      });
    var candleEnter = candle.enter().append('g').attr('transform', function (d) {
    ...
    candleUpdate.select('.extent').attr('y1', function (d) {
      return priceScale(d.low);
    }).attr('y2', function (d) {
      return priceScale(d.high);
    });
    candleUpdate.select('rect').attr('x', -candleWidth / 2).attr('width', candleWidth).attr('y', function (d) {
      return priceScale(Math.max(d.open, d.close));
    }).attr('height', function (d) {
      return Math.abs(priceScale(d.open) - priceScale(d.close)) + 0.5;
    });
    candleUpdate.select('.high').attr('x1', -candleWidth / 4).attr('x2', candleWidth / 4).attr('y1', function (d) {
      return priceScale(d.high);
    }).attr('y2', function (d) {
      return priceScale(d.high);
    });
    candleUpdate.select('.low').attr('x1', -candleWidth / 4).attr('x2', candleWidth / 4).attr('y1', function (d) {
      return priceScale(d.low);
    }).attr('y2', function (d) {
      return priceScale(d.low);
    });
    d3.transition(candle.exit()).attr('transform', function (d) {
      return 'translate(' + xScale(d.startTime) + ')';
    }).style('opacity', 0.000001).remove();
    var bars = gEnter.select('.volumeBars').selectAll('rect').data(lineData, function (d) {
        return d.startTime;
      });
    ...
    }).attr('width', candleWidth / 1.2).attr('height', function (d) {
      return options.height - volumeScale(d.baseVolume);
    }).style('fill', 'url(#gradient)');
    bars.exit().remove();
    gEnter.select('.x.axis').transition().call(xAxis).attr('transform', 'translate(0,' + priceScale.range()[0] + ')');
    gEnter.select('.price.axis').transition().call(priceAxis).attr('transform', 'translate(' + xScale.range()[1] + ', 0)');
  ...
  function showDetails() {
    var z = wrap.style('zoom') || 1, x = d3.mouse(this)[0] / z, tx = Math.max(0, Math.min(options.width + options.margin.left, x)), i = d3.bisect(lineData.map(function (d) {
        return d.startTime;
      }), xScale.invert(tx - options.margin.left)), d = lineData[i], o, h, l, c, v;
    if (d) {
      ...
      details.html('<span class=\'date\'>' + parseDate(d.startTime.local(), chartInterval) + '</span><span><small>O:</small><b>' + o + '</b></span>' + '<span class=\'high\'><small>H:</small><b>' + h + '</b></span>' + '<span class=\'low\'><small>L:</small><b>' + l + '</b></span>' + '<span><small>C:</small><b>' + c + '</b></span>' + '<span class=\'vwap\'><small>VWAP:</small><b>' + a + '</b></span>' + '<span class=\'volume\'><small>Vol:</small><b>' + v + ' ' + baseCurrency + '</b></span>').style('opacity', 1);
      ...
      horizontal.transition().duration(50).attr('x1', xScale(d.startTime)).attr('x2', options.width).attr('y1', priceScale(d.close)).attr('y2', priceScale(d.close));
  ...
  function parseDate(date, increment) {
    if (increment == 'mi')
      return date.format('MMMM D') + ' &middot ' + date.format('hh:mm A') + ' ' + tz(date);
    else if (increment == 'ho')
      return date.format('MMMM D') + ' &middot ' + date.format('hh:mm A') + ' ' + tz(date);
    else if (increment == 'da')
      return date.utc().format('MMMM D') + ' <small>(' + date.utc().format('hh:mm A') + ' UTC)</small>';
    else if (increment == 'mo')
      return date.utc().format('MMMM YYYY') + '<small>UTC</small>';
    else if (increment == 'ye')
      return date.utc().format('YYYY');
    else
      return date.format('MMMM D') + ' &middot ' + date.format('hh:mm:ss A') + ' ' + tz(date);
  }
  function tz(dateInput) {
    var dateObject = dateInput.toDate() || new Date(), dateString = dateObject + '', tzAbbr = dateString.match(/\(([^\)]+)\)$/) || dateString.match(/([A-Z]+) [\d]{4}$/);
    if (tzAbbr) {
