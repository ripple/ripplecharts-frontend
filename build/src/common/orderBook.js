var OrderBook = function (options) {
  ...
  var chart = d3.select('#' + options.chartID).attr('class', 'orderbookChart'), svg, depth, gEnter, xAxis, leftAxis, rightAxis, xTitle, leftTitle, rightTitle, hover, focus, centerline, path, midpoint, status, details, loader, baseCurrency, counterCurrency;
  if (!options.margin)
    options.margin = {
      top: 2,
      right: 60,
  ...
  var xScale = d3.scale.linear(), yScale = d3.scale.linear(), lineData = [];
  ...
  var bidsTable = bookTables.append('table').attr('class', 'bidsTable'), bidsHead = bidsTable.append('thead'), bidsBody = bidsTable.append('tbody');
  bidsHead.append('tr').selectAll('th').data(['Bids']).enter().append('th').attr('class', 'type').attr('colspan', 3).text(function (d) {
    return d;
  });
  bidsHead.append('tr').attr('class', 'headerRow').selectAll('th').data([
    'Total',
    'Size',
    'Bid Price'
  ...
  var asksTable = bookTables.append('table').attr('class', 'asksTable'), asksHead = asksTable.append('thead'), asksBody = asksTable.append('tbody');
  asksHead.append('tr').selectAll('th').data(['Asks']).enter().append('th').attr('class', 'type').attr('colspan', 3).text(function (d) {
    return d;
  });
  asksHead.append('tr').attr('class', 'headerRow').selectAll('th').data([
    'Ask Price',
    'Size',
    'Total'
  ]).enter().append('th').text(function (d) {
  ...
  function handleBook(data, action) {
    ...
    newData.forEach(function (d) {
      ...
      if (action === 'asks') {
        d.price = Amount.from_quality(d.BookDirectory, d.TakerPays.currency(), d.TakerPays.issuer(), {
          base_currency: d.TakerGets.currency(),
          reference_date: new Date()
        });
      } else {
        d.price = Amount.from_quality(d.BookDirectory, d.TakerGets.currency(), d.TakerGets.issuer(), {
          inverse: true,
    ...
    offers.forEach(function (offer, index) {
      if (sum)
        sum = offer.sum = sum.add(offer[type]);
      else
        sum = offer.sum = offer[type];
  ...
  function drawChart() {
    ...
    depth = svg.enter().append('svg').attr('width', options.width + options.margin.left + options.margin.right).attr('height', options.height + options.margin.top + options.margin.bottom).on('mousemove', mousemove);
    gEnter = depth.append('g').attr('transform', 'translate(' + options.margin.left + ',' + options.margin.top + ')');
    ...
    details = chart.append('div').attr('class', 'chartDetails').style('left', options.margin.left + 'px').style('right', options.margin.right + 'px').style('top', options.margin.top - 1 + 'px').style('opacity', 0);
    gEnter.append('rect').attr('class', 'background').attr('width', options.width).attr('height', options.height);
    loader = chart.append('img').attr('class', 'loader').attr('src', 'assets/images/rippleThrobber.png').style('opacity', 0);
    if (isLoading) {
  ...
  function drawData() {
    if (!self.offers.bids || !self.offers.asks)
      return;
    if (!self.offers.bids.length || !self.offers.asks.length) {
      setStatus('No Orders');
      return;
    }
    var bestBid = self.offers.bids[0].showPrice, bestAsk = self.offers.asks[0].showPrice;
    ...
    setStatus('');
    isLoading = false;
    var min = Math.max(d3.min(lineData, function (d) {
        return d.showPrice;
      }), bestBid / 5), max = Math.min(d3.max(lineData, function (d) {
        return d.showPrice;
      }), bestAsk * 5);
    for (var i = 0; i < lineData.length; i++) {
      if (lineData[i].showPrice < min || lineData[i].showPrice > max)
        lineData.splice(i--, 1);
    }
    ...
    path.datum(lineData).transition().attr('d', d3.svg.line().x(function (d) {
      return xScale(d.showPrice);
    }).y(function (d) {
      return yScale(d.showSum);
    }));
  ...
  function mousemove() {
    var z = chart.style('zoom') || 1, tx = Math.max(0, Math.min(options.width + options.margin.left, d3.mouse(this)[0]) / z), i = d3.bisect(lineData.map(function (d) {
        return d.showPrice;
      }), xScale.invert(tx - options.margin.left));
    d = lineData[i];
    if (d && !d.showSum)
      d = d.showPrice < midpoint ? lineData[i - 1] : lineData[i + 1];
    if (d) {
      ...
      details.html('<span>Quantity:<b> ' + quantity + '</b></span><span>Total:<b> ' + d.showSum + ' ' + baseCurrency + '</b></span>' + '<span> @ <b>' + d.showPrice + ' ' + counterCurrency + '</b></span>').style('opacity', 1);
    }
  }
  function redrawBook() {
    if (!self.offers.bids || !self.offers.asks)
    ...
    row.exit().remove();
    row.select('.sum').html(function (offer) {
      return filter(offer.sum);
    });
    row.select('.size').html(function (offer) {
      return filter(offer.TakerPays);
    });
    row.select('.price').html(function (offer) {
      return filter(offer.price);
    });
    row.attr('title', function (d) {
      return d.Account;
    });
  ...
  function valueFilter(amount, opts) {
    return parseFloat(amount.to_human(opts ? opts : {
      precision: 8,
      min_precision: 0,
      max_sig_digits: 8,
      group_sep: false
