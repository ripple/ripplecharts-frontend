networkGraph = function () {
  ...
  var REQUEST_REPETITION_INTERVAL = 8 * 1000;
  ...
  function handleAccountTransactions(err, obj) {
    ...
    if (obj.account == focalNode) {
      updateTransactions(focalNode, true);
    }
  }
  ...
  function txDescription(result) {
    ...
    if (result.meta) {
      for (var i = 0; i < result.meta.AffectedNodes.length; i++) {
        ...
        if (an.ModifiedNode && an.ModifiedNode.LedgerEntryType === 'AccountRoot' && an.ModifiedNode.FinalFields && an.ModifiedNode.FinalFields.Account === result.Account) {
          xrpExpense = {
            before: an.ModifiedNode.PreviousFields.Balance / 1000000,
            after: an.ModifiedNode.FinalFields.Balance / 1000000
          };
          break;
        }
      }
    ...
    if (amount) {
      ...
      div.append('<b>Amount:</b>').append(span).append('<br/>');
      if (amount.issuer)
        div.append('<b>Issuer:</b>').append(clickableAccountSpan(amount.issuer)).append('<br/>');
    }
    div.append('<b>Path:</b>').append(function () {
      var ul = $('<ul/>'), li;
      if (result.Paths) {
        for (var i = 0; i < result.Paths.length; i++) {
          li = $('<li/>');
          li.append(clickableAccountSpan(result.Account)).append(' &rarr; ');
          for (var j = 0; j < result.Paths[i].length; j++) {
            if (result.Paths[i][j].account) {
              li.append(clickableAccountSpan(result.Paths[i][j].account)).append(' &rarr; ');
            }
          }
        ...
        li = $('<li/>');
        li.append(clickableAccountSpan(result.Account)).append(' &rarr; ').append(clickableAccountSpan(result.Destination));
        ul.append(li);
      }
      return ul;
    });
    div.append((result.meta ? '<b>Result:</b> ' + (result.meta.TransactionResult == 'tesSUCCESS' ? '<span>' : '<span style=\'color:#900;\'>') + result.meta.TransactionResult + '</span><br/>' : '') + (xrpExpense || xrpExpense === 0 ? '<b>XRP change:</b> ' + commas(xrpExpense.before) + ' XRP &rarr; ' + commas(xrpExpense.after) + ' XRP (' + (xrpExpense.after >= xrpExpense.before ? '+' : '&ndash;') + commas(Math.round(1000000 * Math.abs(xrpExpense.before - xrpExpense.after)) / 1000000) + ' XRP)<br/>' : '') + (result.date ? '<b>Date:</b> ' + absoluteDateOnly(result.date) + ' ' + absoluteTimeOnly(result.date) + '<br/>' : '') + (result.InvoiceID ? '<b>Invoice ID:</b> <tt>' + result.InvoiceID + '</tt><br/>' : '') + (result.DestinationTag ? '<b>Destination tag:</b> ' + result.DestinationTag + '<br/>' : '') + '<b>Hash:</b> <tt>' + result.hash + '</tt><br/>' + (result.inLedger ? '<b>Ledger:</b> ' + result.inLedger + '<br/>' : '') + '<b>Signing key:</b> <tt>' + result.SigningPubKey + '</tt><br/><b>Signature:</b><br/><div class=\'bigString\' style=\'width:' + result.TxnSignature.length * 4 + 'px;\'>' + result.TxnSignature + '</div>');
    return div;
  }
  ...
  function addConnections(origin, trustLines) {
    ...
    if (currentCurrency != 'XRP') {
      svg.select('g#nodeGroup').select('circle#_' + origin).attr('r', nodeRadius(nodes[nodeMap[origin]]));
      svg.select('g#haloGroup').select('circle#halo_' + origin).attr('r', HALO_MARGIN + nodeRadius(nodes[nodeMap[origin]]));
    }
      ...
      if (!transactionMode && trustLines.length >= MAX_NUTL) {
        ...
        for (var i = 0; i < trustLines.length; i++) {
          ...
          if (linkWasToExisting) {
            svg.select('g#haloGroup').select('circle#halo_' + account).style('display', numberOfUnseenTrustLines(node) > 0 ? 'block' : 'none');
          }
        }
      }
    }
    ...
    svg.select('g#haloGroup').select('circle#halo_' + origin).style('display', numberOfUnseenTrustLines(nodes[nodeMap[origin]]) > 0 ? 'block' : 'none');
    displayingTransactionInPlace = false;
    function goon(link) {
  ...
  var svg = d3.select('#visualization').append('svg:svg').attr('class', 'visual').attr('height', h).attr('pointer-events', 'auto').on('click', function () {
    ...
    }).style('float', 'left').style('margin-right', '10').call(d3.behavior.drag().on('drag', redraw));
  var zoomLevel = 1;
  var translationX = 0;
  var translationY = 0;
  var panOffset = [
  ...
  function panAndZoom() {
    linkGroup.attr('transform', 'translate(' + [
      translationX,
      translationY
    ] + '),scale(' + zoomLevel + ')');
    ...
    haloGroup.attr('transform', 'translate(' + [
      translationX,
      translationY
    ] + '),scale(' + zoomLevel + ')');
    arrowheadGroup.attr('transform', 'translate(' + [
      translationX,
      translationY
    ] + '),scale(' + zoomLevel + ')');
  }
  ...
  function defineRadialGradient(name, innerColor, outerColor) {
    var radGrad = defs.append('radialGradient').attr('id', name).attr('fx', '50%').attr('fy', '50%').attr('r', '100%').attr('spreadMethod', 'pad');
    radGrad.append('stop').attr('offset', '0%').attr('stop-color', innerColor).attr('stop-opacity', '1');
    radGrad.append('stop').attr('offset', '100%').attr('stop-color', outerColor).attr('stop-opacity', '1');
  }
  ...
  var force = d3.layout.force().size([
      ...
      710
    ]).linkDistance(80).linkStrength(function (d) {
      if (currentCurrency == 'XRP' || currentCurrency == d.currency) {
        return d.strength * 0.25;
      } else {
        return 0;
      }
    }).friction(0.5).charge(-1500).nodes([]).links([]).start();
  ...
  function borderColor(cur, colorDegree) {
    if (colorDegree === 0) {
      ...
      return COLOR_TABLE[cur][colorDegree - 1][1];
    }
  }
  function findCur(d) {
    ...
    if (cur != 'XRP') {
      if (!d.balances[cur]) {
        cur = '__Z';
      } else if (d.balances[cur] < 0) {
        cur = '__N';
      } else if (!COLOR_TABLE.hasOwnProperty(cur)) {
        cur = '___';
      }
    }
    return cur;
  ...
  function darkenNodeFunction(colorDegree) {
    return function (d) {
      var cur = findCur(d);
      d3.select(d3.event.target).style('fill', 'url(#gradient' + cur + colorDegree + ')').style('stroke-width', colorDegree === 0 ? 5 : 0.5).style('stroke', function (d) {
        var cur = findCur(d);
  ...
  function darkenAddress(address) {
    if (typeof degreeMap[address] != 'undefined') {
      var colorDegree = Math.min(degreeMap[address], 3);
      var cur = findCur(force.nodes()[nodeMap[address]]);
      nodeGroup.select('#_' + address).style('fill', 'url(#gradient' + cur + colorDegree + ')').style('stroke-width', colorDegree === 0 ? 5 : 0.5).style('stroke', function (d) {
        var cur = findCur(d);
        return borderColor(cur, colorDegree);
      });
    }
  ...
  function isLineInvisible(source, target) {
    for (var j = 0; j < le_links.length; j++) {
      if (le_links[j].source.account.Account == source && le_links[j].target.account.Account == target || le_links[j].source.account.Account == target && le_links[j].target.account.Account == source) {
        return false;
      }
    }
    return true;
  ...
  function colorNodes(nodeSelection, colorDegree) {
    nodeSelection.style('fill', function (d) {
      ...
      var cur = findCur(d);
      return borderColor(cur, colorDegree);
    }).style('stroke-width', 0.5).on('mouseover', lightenNodeFunction(colorDegree)).on('mouseout', darkenNodeFunction(colorDegree));
    if (colorDegree === 0) {
  ...
  function fadeLinks(address) {
    for (var i = 0; i < le_links.length; i++) {
      var link = le_links[i];
      if (link.source.account.Account == address || link.target.account.Account == address) {
        le_links[i].opacity = 1;
      } else {
        le_links[i].opacity = 0.3;
      }
    }
    svg.select('g#linkGroup').selectAll('line.static').data(le_links).style('opacity', function (d) {
      return d.opacity;
    });
  }
  ...
  function animateLink(onOrOff, speed, from, to, cur, callback) {
    if (typeof nodeMap[from] == 'undefined' || typeof nodeMap[to] == 'undefined') {
      setTimeout(callback, 10 / speed);
    } else {
      var animator = $('#' + from + '_' + to + '_' + cur);
      if (animator.length === 0) {
        animatorLinks.push({
          source: nodes[nodeMap[from]],
          target: nodes[nodeMap[to]],
          value: 100,
          currency: currency,
          strength: 0
        });
        var alink = svg.select('g#linkGroup').selectAll('line.animator').data(animatorLinks).enter().append('svg:line').attr('x1', function (d) {
            return d.source.x;
          }).attr('y1', function (d) {
            return d.source.y;
          }).attr('x2', function (d) {
            ...
            return d.target.y;
          }).attr('class', 'animator').attr('id', from + '_' + to + '_' + cur).style('stroke', function (d) {
            return HIGH_SATURATION_COLORS[cur];
          }).style('z-index', '2').style('stroke-dasharray', '0,999999').attr('stroke-width', 10);
  ...
  function animateTransaction(tx) {
    ...
    if (tx.SendMax && tx.SendMax.currency) {
      ...
      if (!HIGH_SATURATION_COLORS.hasOwnProperty(initialCur)) {
        initialCur = '___';
      }
    } else {
      initialCur = 'XRP';
    ...
    if (amount.currency) {
      ...
      if (!HIGH_SATURATION_COLORS.hasOwnProperty(finalCur)) {
        finalCur = '___';
      }
    } else {
      finalCur = 'XRP';
    ...
    function animatePath(onOrOff, i) {
      ...
      function animatePathLink(j) {
        if (j == path.length) {
          animateLink(onOrOff, speed, lastNode, tx.Destination, finalCur, function () {
            shine(onOrOff, tx.Destination, finalCur);
            if (onOrOff) {
              shine(false, tx.Account);
              animatePath(false, i);
            }
          });
        } else {
          ...
          if (path[j].currency) {
            cur = path[j].currency;
            if (!HIGH_SATURATION_COLORS.hasOwnProperty(cur)) {
              cur = '___';
            }
          } else {
            cur = 'XRP';
  ...
  function addNodes(degree) {
    ...
    var node = svg.select('g#nodeGroup').selectAll('circle.node').data(nodes).enter().append('svg:circle').attr('class', 'node').attr('id', function (d) {
        return '_' + d.account.Account;
      }).attr('cx', function (d) {
        return d.x;
      }).attr('cy', function (d) {
        return d.y;
      }).attr('r', nodeRadius).attr('title', function (d) {
        return d.account.Account;
      }).attr('pointer-events', 'all').style('cursor', 'pointer').on('touchstart', function () {
      }).on('touchmove', function (d) {
        lastNodeTouched = d.account.Account;
      }).on('touchend', function (d) {
        if (lastNodeTouched != d.account.Account) {
          stopExpandResume(d);
          lastNodeTouched = d.account.Account;
        } else {
          lastNodeTouched = '';
    ...
    var link = svg.select('g#linkGroup').selectAll('line.static').data(force.links()).enter().append('svg:line').attr('class', 'static').style('opacity', function (d) {
        return d.opacity ? d.opacity : 1;
    ...
    function arrowheadPath(radius, theta) {
      ...
      return 'M 0 0 L ' + rCosTheta + ' ' + rSinTheta + ' L ' + (rCosTheta + rSinTheta * Math.sqrt(3)) + ' 0 L ' + rCosTheta + ' ' + -rSinTheta + ' z';
    }
    var arrowhead = svg.select('g#arrowheadGroup').selectAll('path.arrowhead').data(force.links()).enter().append('svg:path').attr('class', 'arrowhead');
    ...
    function thetaValue(value) {
      return value / (1 + value) * 1.04719755;
    }
    function setArrowheads(selection) {
      return selection.attr('id', function (d) {
        return 'arrow_' + d.source.account.Account;
      }).attr('transform', function (d) {
        var position = d.source;
        return 'translate(' + position.x + ',' + position.y + '), rotate(' + angle(d.source, d.target) + ',0,0)';
      }).attr('d', function (d) {
        return arrowheadPath(parseFloat($('#_' + d.source.account.Account).attr('r')), thetaValue(d.value));
      }).style('display', function (d) {
        return isLinkVisible(d) ? 'block' : 'none';
      });
    }
    var halo = svg.select('g#haloGroup').selectAll('circle.halo').data(nodes).enter().append('svg:circle').attr('class', 'halo').attr('id', function (d) {
        return 'halo_' + d.account.Account;
      }).attr('cx', function (d) {
        return d.x;
      }).attr('cy', function (d) {
        return d.y;
      }).attr('r', function (d) {
        return HALO_MARGIN + nodeRadius(d);
      });
    force.start();
  ...
  function roundNumber(number) {
    ...
    if (number === 0 || man < 100000 && man > 0.00001) {
      return commas(Math.round(number * 100) / 100);
    } else {
      var oom = Math.floor((Math.log(man) + 1e-14) / Math.LN10);
  ...
  function updateInformation(address) {
    ...
    for (var i = 0; i < currencies.length; i++) {
      ...
      tr = $('<tr style="cursor:' + (trustLinesForCur.length ? 'pointer' : 'auto') + '" sublistid="' + cur + '" numberofsubrows="' + trustLinesForCur.length + '">' + '<td class="circlecell"><svg width="22" height="22">' + '<circle cx="11" cy="11" r="11" style="fill:' + COLOR_TABLE[COLOR_TABLE.hasOwnProperty(cur) ? cur : '___'][0][1] + ';"></circle>' + '</svg></td>' + '<td class="light small mediumgray">' + cur + '</td>' + '<td class="bold amount">' + commas(balances[cur]) + '</td>' + '<td class="light expander">' + (trustLinesForCur.length ? '<span id="' + cur + 'Expander">+</span></td>' : '&nbsp;') + '</tr>');
      if (trustLinesForCur.length)
        tr.click(rowClick);
      $('#balanceTable').append(tr);
      if (trustLinesForCur.length) {
        $('#balanceTable').append('<tr class="innertablecontainer" id="' + cur + '">' + '<td colspan=4>' + '<div id="' + cur + 'Inner">' + '<table class="innertable" style="table-layout:fixed;" id="' + cur + 'InnerTable">' + '</table></div></td></tr>');
        $('#' + cur + 'InnerTable').append('<tr>' + '<th class="light midsize mediumgray" style="width:50%;">Address</th>' + '<td class="light midsize mediumgray" style="width:16%;">Min</td>' + '<td class="light midsize mediumgray center" style="width:16%">Balance</td>' + '<td class="light midsize mediumgray right" style="width:16%;">Max</td>' + '</tr>');
  ...
  function updateTransactions(address, appending) {
    ...
    if (nodes[nodeMap[address]].transactions) {
      for (var i = $('#transactionTable tr').length; i < nodes[nodeMap[address]].transactions.length; i++) {
          ...
          if (tx.Account == address) {
            ...
            if (affectedKeys.length == 2) {
              ...
              if (positive * negative > 0) {
                ...
                amount = affectedKeys[posKey] == 'XRP' ? positive : {
                  value: positive,
                  currency: affectedKeys[posKey]
                };
                secondAmount = affectedKeys[negKey] == 'XRP' ? -negative : {
                  value: -negative,
                  currency: affectedKeys[negKey]
                };
              }
            } else {
        ...
        if (transactionType == 'send' || transactionType == 'receive' || transactionType == 'intermediate') {
        ...
        if (amount)
          span.append('<span class="bold amount small" >' + commas(amount) + '</span> <span class="light small darkgray" style="margin-right:5px">' + currency + '</span></span>');
        if (secondAmount)
          span.append(' <i class="light small darkgray" style="margin-right:5px">for</i> <span ' + (secondAissuer && !(transactionType == 'trustin' || transactionType == 'trustout') ? 'title="' + secondAissuer + '"' : '') + '><span class="bold amount small">' + commas(secondAmount) + '</span> <span class="light small darkgray" style="margin-right:5px">' + secondCurrency + '</span>');
        span.append(agoDate(tx.date));
        td.append(span);
        if (counterparty)
          td.append(clickableAccountSpan(counterparty).css({
            display: 'block',
            'margin-top': '3px',
            overflow: 'hidden',
      ...
      if (!nodes[nodeMap[address]].transactionsFinished) {
  ...
  function changeCurrency(newCurrency) {
    ...
    if (newCurrency == '___') {
      ...
      svg.select('g#haloGroup').selectAll('circle.halo').attr('r', function (d) {
        return HALO_MARGIN + nodeRadius(d);
      });
  ...
  $('#otherCurrency').focus(function () {
    focusOtherCurrency(this);
  }).blur(function () {
    blurOtherCurrency(this);
  });
  ...
  function isConnected() {
    var server;
    return remote._connected && (server = remote._getServer()) && Date.now() - server._lastLedgerClose <= 1000 * 20;
  }
  function init() {
