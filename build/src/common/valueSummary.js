var ValueSummary = function (options) {
  var self = this, outer = options.id ? d3.select('#' + options.id).attr('class', 'valueSummary') : d3.select('body').append('div').attr('class', 'valueSummary');
  var inner = outer.append('div').attr('class', 'inner');
  var width = parseInt(outer.style('width'), 10), height = parseInt(outer.style('height'), 10) || width;
  ...
  radius -= margin.top;
  var chart = inner.append('svg').attr('width', radius * 2.3).attr('height', radius * 2.3).append('g').attr('transform', 'translate(' + (radius + margin.left) + ',' + (radius + margin.top) + ')');
  var toggle = outer.append('label').attr('class', 'xrpToggle');
  var hideXRP = true;
  toggle.append('input').attr('type', 'checkbox').property('checked', !hideXRP).on('click', function () {
  ...
  var labelArc = d3.svg.arc().outerRadius(radius * 1.15).innerRadius(radius);
  ...
  this.load = function (z, ex, xrpToggle) {
    ...
    data.forEach(function (d) {
      if (d.currency == 'XRP')
        XRPObj = d;
      d.percent = total ? d.convertedAmount / total * 100 : 0;
    });
    if (!XRPObj)
      data.push({
        currency: 'XRP',
      ...
      data.forEach(function (d) {
        if (d.currency == 'XRP')
          d.convertedAmount = 0;
        d.percent = adjusted ? d.convertedAmount / adjusted * 100 : 0;
      });
    } else {
      XRPObj.convertedAmount = XRPObj.amount || 0;
      XRPObj.percent = total ? XRPObj.amount / total * 100 : 0;
    }
    data.sort(function (a, b) {
      var i1 = a.base ? a.base.currency + a.base.issuer : a.currency + a.issuer || 'Z';
      var i2 = b.base ? b.base.currency + b.base.issuer : b.currency + b.issuer || 'Z';
    ...
    var pathUpdate = chart.selectAll('path').style('fill', function (d, i) {
        return color(d);
      }).style('stroke', function (d, i) {
        return color(d);
      }).style('stroke-width', '.35px').transition().duration(750).attrTween('d', arcTween).attr('id', function (d, i) {
        return 'arc_' + i;
      }).each('end', function () {
        transitioning = false;
      });
    ...
    label.html(function (d) {
      if (!d.data.currency && !d.data.base)
        return '';
      if (!d.data.convertedAmount)
        return '';
      if (d.data.percent < 2)
        return '';
      var label;
    ...
    }).transition().duration(500).style('top', function (d) {
  ...
  function showTooltip(d, i) {
    ...
    if (value)
      tooltip.append('div').attr('class', 'value').html('<label>Value:</label> ' + value + ' <small>' + exchange.currency + '</small>');
    tooltip.append('div').attr('class', 'amount').html('<label>Amount:</label> ' + amount + ' <small>' + currency + '</small>');
    if (count)
      tooltip.append('div').attr('class', 'count').html('<label>Count:</label> ' + count);
