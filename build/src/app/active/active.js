angular.module('ripplecharts.activeAccounts', [
  ...
  '$scope',
  function ActiveAccountsCtrl($scope) {
    var base = store.session.get('traderBase') || store.get('traderBase') || {
        'currency': 'USD',
    ...
    var loaded = false, dropdownB = ripple.currencyDropdown().selected(counter).on('change', function (d) {
        if (loaded) {
          counter = d;
          loadPair();
        }
      }), dropdownA = ripple.currencyDropdown().selected(base).on('change', function (d) {
        if (loaded) {
          base = d;
          loadPair();
        }
