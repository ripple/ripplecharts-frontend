...
function Topology($http) {
  ...
  self.formatUptimes = function(nodes) {
    function format(time) {
      ...
      if (days > 0) {
        return {name: dayName, time: days * 60 * 60 * 24}
      } else if (hours > 0) {
        return {name: hourName, time: hours * 60 * 60}
      } else if (minutes > 0) {
        return {name: minName, time: minutes * 60}
      } else {
        return {name: secName, time: seconds}
      }
    }
  ...
  self.mergeOldAndNew = function(newNodes, oldNodes) {
    ...
    var ret = _.map(newNodes, function(p) {
      return {
        new: p,
        old: oldNodesByPubKey[p.node_public_key]
      }
    })
    return ret
  }
  ...
  self.produce = function(options) {
    ...
    graph.force = d3.layout.force()
      ...
      .on('tick', function() {
        ...
        graph.nodes.attr('cx', function(d) {
          return d.x
        })
        .attr('cy', function(d) {
          return d.y
  ...
  self.update = function(data) {
    ...
    var extent = d3.extent(data.nodes, function(d) {
      d.connections = Number(d.inbound_count || 0) +
        Number(d.outbound_count || 0)
  ...
  // changes the weight of graph nodes
  // from uptime to connections (or vice versa)
  this.weight = function(weight_by) {
    d3.selectAll('circle.graph-node').each(function(d) {

      var node = d3.select(this)
      var scale = weight_by === 'uptime' ?
          uScale : cScale
  ...
  this.color = function(versionColor) {
    d3.selectAll('circle.graph-node').each(function(d) {
      ...
      .style('stroke', versionColor(d.version))
    })
  }
}


function TopologyMap($http) {
  var self = this
  var parent
  var svg
  var nodes
  ...
  .on('zoom', function() {
    ...
    .attr('transform', function() {
      ...
      return 'translate(' + trans.translate[0] + ',' +
        trans.translate[1] + ')scale(' + Math.pow(1 / e.scale, 0.7) + ')'
    })
    .attr('_r', function() {
      // only update _r if the node isn't highlighted
      // prevents node size from being
      // stuck if the user zooms while highlighting a node
      var is_highlighted = d3.select(this)
      .attr('class')
      .indexOf('highlight') > -1

      return is_highlighted ?
        d3.select(this).attr('_r') : d3.select(this).attr('r')
    })
  })
  ...
  // if the node has an invalid/unknown location,
  // then place it in the appropriate zone
  // this function generates the x and y offsets
  // so that those nodes are placed in neat rows and cols
  function get_offset(invalid_count) {
    var x_dim = 10
    var y_dim = 12
    var x_offset = 10
    var y_offset = h - 25 // 420

    // variable for which column the node should be placed in
    var placement = x_dim * invalid_count
    // if the node placement is beyond the edge of the box,
  ...
  self.draw = function(properties) {
    ...
    // alternative options: stereographic,
    // orthographic (globe), equirectangular,
    // albers, transverseMercator
    ...
    // transparent rectangle so click to drag works
    // on the entire map, not just the land
    nodes.append('rect')
       .attr('x', 0)
       .attr('y', 0)
       .attr('width', w)
      ...
      .attr('transform', function(d) {
        ...
        if (d.lat && d.long) {
          return 'translate(' + projection([d.long, d.lat]) + ')' +
            'scale(' + Math.pow(1 / zoom.scale(), 0.7) + ')'
        }
