var TotalHistory = function (options) {
  ...
  var customTo = $('#datepicker_to').datepicker({
    maxDate: '+0d',
    minDate: '',
    onSelect: loadCustomRange
  });

  var customFrom = $('#datepicker_from').datepicker({
    maxDate: '-2d',
    onSelect: loadCustomRange
  });

  function loadCustomRange(dateText) {
  ...
  function filterByIssuer() {
    ...
    for (date in cache.byDate) {
      ...
      source.components.forEach(function(c) {
        ...
        var currency = c.currency + '.' + (c.issuer || '');
        keys[currency] = {
          label: currency,
          currency: c.currency,
          issuer: c.issuer,
    ...
    for (date in cache.byDate) {
      ...
      source.components.forEach(function(c) {
        ...
        list[currency] = c.converted_amount;
      });
  ...
  function formatData(data) {
    ...
    data.forEach(function(d) {
      var date = moment(d.start_time);
      result.push({
        date: date,
        y: d.total ? Number(d.total.toFixed(0)) : 0
      });
    });
