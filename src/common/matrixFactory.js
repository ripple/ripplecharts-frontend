angular.module('matrixFactory', []).factory('matrixFactory', [function () {
  ...
  var chordMatrix = function () {
    ...
    matrix.chords = function () {
      return chordLayout.chords().map(function (chord) {
        ...
        currency = chord.source._id.split('.')[0];
        return chord;
      }).filter(function(chord) {
        return chord.source.value.value ? true : false;
      });
    };
    ...
    matrix.read = function (d) {
      ...
      if (d.source) {
        m.sourceName  = d.source._id;
        m.sourceData  = d.source.value;
        m.sourceValue = +d.source.value;
        m.sourceTotal = _matrix[d.source.index].reduce(function (k, n) { return k + n; }, 0);
        m.targetName  = d.target._id;
        m.targetData  = d.target.value;
        m.targetValue = +d.target.value;
        //m.ttotal = _matrix[d.target.index].reduce(function (k, n) { return k + n; }, 0);
      } else {
        g = indexHash[d._id];
        m.groupName  = g.name;
        m.groupData  = g.data;
        m.groupValue = d.value;
      }
      m.total = _matrix.reduce(function (m1, n1) {
